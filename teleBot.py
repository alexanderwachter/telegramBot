...
def search_rooms(name):
    ...
    for tr in rows:
        cols = tr.findAll('td')
        if(len(cols) > 5):
            rooms.append((cols[3].find(text=True), cols[5].find(text=True), url + cols[5].find('a')['href']))
    return rooms
...
async def on_chat_message(msg):
    ...
    if(msg.get('chat').get('last_name')):
        info += ' ' + msg['chat']['first_name'] + ' ' + msg['chat']['last_name']
...
def on_inline_query(msg):
    def compute():
        ...
        articles = []
        for room in rooms:
            if room[0] is None or room[1] is None or room[2] is None: continue
...
def on_chosen_inline_result(msg):
    ...
    on_chosen_inline_result.log.info('Chosen:', result_id, from_id, query_string)
